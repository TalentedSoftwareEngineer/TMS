...
import {
    ADMIN_DATA_OPERATION, NUMBER_STATUS,
    PAGE_OPERATION,
    PROGRESSING_STATUS,
...
export class PointerRecordService {
    constructor(
    ...
    async retrieve(ro: string, num: string, effDtTm: string, profile: AuthorizedUserProfile, isAct: boolean) {
        ...
        let message = ""
        let response = await this.tfnRegistryApiService.retrievePointerRecord(ro, num, profile, eff_dt_tm)
        if (response == null) {
            ...
            while (response == null || response.tmplId == null) {
                ...
                response = await this.tfnRegistryApiService.retrievePointerRecordByReqId(ro, reqId, profile)
            }
        ...
        if (message.length > 0) {
            if (isAct) {
                ...
                const activity = await this.createActivity(profile, PAGES.PointAdminData, PAGE_OPERATION.RETRIEVE, current_utc_time,
        ...
        if (isAct) {
            ...
            this.saveActivityResult(profile, activity.id, TASK_TYPE.PAD, TASK_ACTION.RETRIEVE, num, current_utc_time, activity.status, response.ctrlRespOrgId, undefined, response.effDtTm)
        }
    ...
    async lock(ro: string, body: any, profile: AuthorizedUserProfile) {
        ...
        let response = await this.tfnRegistryApiService.lockPointerRecord(ro, body, profile)
        if (response == null) {
            message = MESSAGES.EMPTY_RESPONSE
            ...
            while (response == null || response.errList != null) {
                await DataUtils.sleep(100)
                response = await this.tfnRegistryApiService.lockPointerRecordByReqId(ro, reqId, profile)
            }
    ...
    async unlock(ro: string, body: any, profile: AuthorizedUserProfile) {
        ...
        let response = await this.tfnRegistryApiService.unlockPointerRecord(ro, body, profile)
        if (response == null) {
            message = MESSAGES.EMPTY_RESPONSE
            ...
            while (response == null || response.errList != null) {
                await DataUtils.sleep(100)
                response = await this.tfnRegistryApiService.unlockPointerRecordByReqId(ro, reqId, profile)
            }
    ...
    async copy(ro: string, body: any, profile: AuthorizedUserProfile) {
        ...
        notify.page = PAGES.PointAdminData
        ...
        if (response == null) {
            ...
            while (response == null || response.effDtTm == null) {
                await DataUtils.sleep(100)
                response = await this.tfnRegistryApiService.copyPointerRecordByReqId(ro, reqId, profile)
            }
        ...
        // save activity, task
        const activity = await this.createActivity(profile, PAGES.PointAdminData, PAGE_OPERATION.COPY, sub_dt_tm,
            1, result == null ? 0 : 1, result == null ? PROGRESSING_STATUS.FAILED : PROGRESSING_STATUS.SUCCESS)

        if (result == null) {
            await this.saveActivityResult(profile, activity.id, TASK_TYPE.PAD, TASK_ACTION.COPY, body.tgtNum, sub_dt_tm, activity.status,
                body.ctrlRespOrgId, message, body.tgtEffDtTm, body.tmplName, undefined,
        ...
        await this.saveActivityResult(profile, activity.id, TASK_TYPE.PAD, TASK_ACTION.COPY, body.tgtNum, sub_dt_tm, body.cmd == ADMIN_DATA_OPERATION.SUBMIT ? TASK_STATUS.PENDING : TASK_STATUS.SAVED,
    ...
    async transfer(ro: string, body: any, profile: AuthorizedUserProfile) {
        ...
        if (response == null) {
            ...
            while (response == null || response.effDtTm == null) {
                await DataUtils.sleep(100)
                response = await this.tfnRegistryApiService.transferPointerRecordByReqId(ro, reqId, profile)
            }
        ...
        // save activity, task
        const activity = await this.createActivity(profile, PAGES.PointAdminData, PAGE_OPERATION.TRANSFER, sub_dt_tm,
            1, result == null ? 0 : 1, result == null ? PROGRESSING_STATUS.FAILED : PROGRESSING_STATUS.SUCCESS)

        if (result == null) {
            await this.saveActivityResult(profile, activity.id, TASK_TYPE.PAD, TASK_ACTION.TRANSFER, body.num, sub_dt_tm, activity.status,
                body.ctrlRespOrgId, message, body.tgtEffDtTm, body.tmplName, undefined,
        ...
        await this.saveActivityResult(profile, activity.id, TASK_TYPE.PAD, TASK_ACTION.TRANSFER, body.num, sub_dt_tm, body.cmd == ADMIN_DATA_OPERATION.SUBMIT ? TASK_STATUS.PENDING : TASK_STATUS.SAVED,
            body.ctrlRespOrgId, message, response.effDtTm, body.tmplName, undefined,
            undefined, body.srcEffDtTm)
    ...
    async disconnect(ro: string, body: any, profile: AuthorizedUserProfile) {
        ...
        if (response == null) {
            ...
            while (response == null || response.effDtTm == null) {
                await DataUtils.sleep(100)
                response = await this.tfnRegistryApiService.disconnectPointerRecordByReqId(ro, reqId, profile)
            }
        ...
        const activity = await this.createActivity(profile, PAGES.PointAdminData, PAGE_OPERATION.DISCONNECT, sub_dt_tm,
            1, result == null ? 0 : 1, result == null ? PROGRESSING_STATUS.FAILED : PROGRESSING_STATUS.SUCCESS)

        if (result == null) {
            await this.saveActivityResult(profile, activity.id, TASK_TYPE.PAD, TASK_ACTION.DISCONNECT, body.num, sub_dt_tm, activity.status,
                body.ctrlRespOrgId, message, body.tgtEffDtTm, body.tmplName, undefined,
                undefined, body.srcEffDtTm)
        ...
        await this.saveActivityResult(profile, activity.id, TASK_TYPE.PAD, TASK_ACTION.DISCONNECT, body.num, sub_dt_tm, body.cmd == ADMIN_DATA_OPERATION.SUBMIT ? TASK_STATUS.PENDING : TASK_STATUS.SAVED,
            body.ctrlRespOrgId, message, response.effDtTm, body.tmplName, undefined,
            undefined, body.srcEffDtTm)
    ...
    async create(ro: string, body: any, profile: AuthorizedUserProfile) {
        ...
        // save activity, task
        const activity = await this.createActivity(profile, PAGES.PointAdminData, PAGE_OPERATION.CREATE, sub_dt_tm,
            1, result == null ? 0 : 1, result == null ? PROGRESSING_STATUS.FAILED : PROGRESSING_STATUS.SUCCESS)

        if (result == null) {
            await this.saveActivityResult(profile, activity.id, TASK_TYPE.PAD, TASK_ACTION.CREATE, body.num, sub_dt_tm, activity.status,
                body.newRespOrgId, message, body.effDtTm, body.tmplName, undefined)
        ...
        await this.saveActivityResult(profile, activity.id, TASK_TYPE.PAD, TASK_ACTION.CREATE, body.num, sub_dt_tm, body.cmd == ADMIN_DATA_OPERATION.SUBMIT ? TASK_STATUS.PENDING : TASK_STATUS.SAVED,
            body.newRespOrgId, message, response.effDtTm, body.tmplName, undefined)
    ...
    async update(ro: string, body: any, profile: AuthorizedUserProfile) {
        ...
        // save activity, task
        const activity = await this.createActivity(profile, PAGES.PointAdminData, PAGE_OPERATION.UPDATE, sub_dt_tm,
            1, result == null ? 0 : 1, result == null ? PROGRESSING_STATUS.FAILED : PROGRESSING_STATUS.SUCCESS)

        if (result == null) {
            await this.saveActivityResult(profile, activity.id, TASK_TYPE.PAD, TASK_ACTION.UPDATE, body.num, sub_dt_tm, activity.status,
        ...
        await this.saveActivityResult(profile, activity.id, TASK_TYPE.PAD, TASK_ACTION.UPDATE, body.num, sub_dt_tm, body.cmd == ADMIN_DATA_OPERATION.SUBMIT ? TASK_STATUS.PENDING : TASK_STATUS.SAVED,
            undefined, message, body.effDtTm, body.tmplName, undefined)
    ...
    async delete(ro: string, num: string, effDtTm: string, recVersionId: string, profile: AuthorizedUserProfile) {
        ...
        // save activity, task
        const activity = await this.createActivity(profile, PAGES.PointAdminData, PAGE_OPERATION.DELETE, sub_dt_tm,
            1, result == null ? 0 : 1, result == null ? PROGRESSING_STATUS.FAILED : PROGRESSING_STATUS.SUCCESS)

        if (result == null) {
            await this.saveActivityResult(profile, activity.id, TASK_TYPE.PAD, TASK_ACTION.DELETE, num, sub_dt_tm, activity.status,
        ...
        await this.saveActivityResult(profile, activity.id, TASK_TYPE.PAD, TASK_ACTION.DELETE, num, sub_dt_tm, TASK_STATUS.PENDING,
            "", message, effDtTm, undefined, undefined)
    ...
    async convert(ro: string, body: any, profile: AuthorizedUserProfile) {
        ...
        if (response == null) {
            ...
            while (response == null || response.effDtTm == null) {
                await DataUtils.sleep(100)
                response = await this.tfnRegistryApiService.convertPointerRecordToCustomerRecordByReqId(ro, reqId, profile)
            }
        ...
        // save activity, task
        const activity = await this.createActivity(profile, PAGES.PointAdminData, PAGE_OPERATION.CONVERT, sub_dt_tm,
            1, result == null ? 0 : 1, result == null ? PROGRESSING_STATUS.FAILED : PROGRESSING_STATUS.SUCCESS)

        if (result == null) {
            await this.saveActivityResult(profile, activity.id, TASK_TYPE.PAD, TASK_ACTION.CONVERT, body.tfNum, sub_dt_tm, activity.status,
                body.ctrlRespOrgId, message, body.tgtEffDtTm, undefined, undefined,
                undefined, body.srcEffDtTm)
        ...
        await this.saveActivityResult(profile, activity.id, TASK_TYPE.PAD, TASK_ACTION.CONVERT, body.tfNum, sub_dt_tm, body.cmd == ADMIN_DATA_OPERATION.SUBMIT ? TASK_STATUS.PENDING : TASK_STATUS.SAVED,
            body.ctrlRespOrgId, message, response.effDtTm, undefined, undefined,
            undefined, body.srcEffDtTm)
