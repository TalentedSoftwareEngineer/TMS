...
export const getFilter = (active: string, direction: string, size: number, page: number, filterValue: string, date1: any, date2: any, keys: string[], val?: string, customerFilter?: any, numberFields?: string) => {
  let filter = 'filter='
  let obj: FilterObj = {
    limit: size,
    skip: (page - 1) * size
  ...
  if (date1 && date2) {
    created = {
      'updated_at': { between: [date1, date2] }
    }
...
export const getCountWhere = (filterValue: string, date1: string, date2: string, keys: string[], customerFilter?: any, numberFields?: string) => {
  let obj: FilterObj = {
  }
  let sobj: Record<string, any> = {}
  let rkeys = []
