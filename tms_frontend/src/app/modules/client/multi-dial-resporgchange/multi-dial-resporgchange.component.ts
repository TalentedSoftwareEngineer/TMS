...
export class MultiDialResporgchangeComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    if(this.store.getUser().permissions?.includes(PERMISSIONS.MULTI_DIAL_NUMBER_RESP_ORG_CHANGE)) {
    } else {
      // no permission
